...
@section('content')
    ...
    <div class="container my-5">
        <div class="d-flex justify-content-between">
            <h2 class="mb-4">{{__('company/companyLang.titleListJob')}}</h2>

            <div>
                <form class="" role="search" action="{{route('company.searchJobs')}}" method="GET">
                    <div class="d-md-flex dropdown text-end">
                        <input class="form-control mb-2 mb-md-0 me-md-2" name="search" placeholder="{{__('company/companyLang.searchPHListJob')}}" value="{{ request('search') }}">
                        ...
                        <button class="btn btn-outline-primary dropdown-toggle me-md-2 mb-2 mb-md-0" type="button" id="dropdownMenuButton1" onclick="toggleDropdown()">
                            {{__('company/companyLang.filterListJob')}}
                        </button>
                        ...
                        <ul class="dropdown-menu mt-md-5  p-2" id="dropdownMenu" aria-labelledby="dropdownMenuButton" >
                                ...
                                <ul class="list-unstyled">
                                    ...
                                    <li><input class="form-check-input" name="job_type[]" type="checkbox" value="Full Time" id="checkFulltime"
                                        ...
                                        <label class="form-check-label" for="checkFulltime">
                                            {{__('company/companyLang.fullTimeListJob')}}
                                        </label>
                                    </li>
                                    <li><input class="form-check-input" name="job_type[]" type="checkbox" value="Part Time" id="checkParttime"
                                        ...
                                        <label class="form-check-label" for="checkParttime">
                                            {{__('company/companyLang.partTimeListJob')}}
                                        </label>
                                    </li>
                                    ...
                                    <li><input class="form-check-input" name="job_type[]" type="checkbox" value="Internship" id="checkInternship"
                                        ...
                                        <label class="form-check-label" for="checkInternship">
                                            {{__('company/companyLang.internshipListJob')}}
                                        </label>
                                    </li>
                            ...
                            <li class="form-group">
                                <ul class="list-unstyled">
                                    <label class="fw-bold">{{__('company/companyLang.statusListJob')}}</label>
                                    <li><input class="form-check-input" name="is_active[]" type="checkbox" value="1" id="checkOpen"
                                        ...
                                        <label class="form-check-label" for="checkOpen">
                                            {{__('company/companyLang.openListJob')}}
                                        </label>
                                    </li>
                                    <li><input class="form-check-input" name="is_active[]" type="checkbox" value="0" id="checkOpen"
                                        ...
                                        <label class="form-check-label" for="checkOpen">
                                            {{__('company/companyLang.closedListJob')}}
                                        </label>
                                    </li>
                                </ul>
                            </li>
                        ...
                        <button class="btn btn-outline-success mb-2 mb-md-0" type="submit">{{__('company/companyLang.searchListJob')}}</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="mt-4 mb-3">
            <a href="{{route('company.createJobPage')}}" class="btn btn-success mb-3">{{__('company/companyLang.addNewJobListJob')}}</a>
        </div>
        <div class="row">
            @forelse ($jobs as $job)
                <div class="col-md-6 col-lg-4 mb-4">
                    <div class="card shadow h-100">
                        ...
                        <div class="card-body d-flex flex-column align-items">
                            ...
                            <p class="mb-1"><strong>{{__('company/companyLang.companyListJob')}}</strong> {{Auth::user()->name}}</p>
                            <p class="mb-1"><strong>{{__('company/companyLang.locationListJob')}} </strong>{{$job->address}}</p>
                            <p class="text-truncate mb-1"><strong>{{__('company/companyLang.descriptionListJob')}} </strong>{{$job->description}}</p>
                            <p class="mb-1"><strong>{{__('company/companyLang.personInChargeListJob')}} </strong>{{$job->person_in_charge}}</p>
                            <p class="mb-1"><strong>{{__('company/companyLang.lastUpdatedListJob')}} </strong>{{$job->updated_at->format('d F Y')}}</p>

                            <a href="{{route('company.job', ['job' => $job])}}" class="btn btn-primary mt-auto">{{__('company/companyLang.detailsListJob')}}</a>
                        </div>
                    </div>
                </div>
            @empty
                <div class="alert alert-danger">
                    {{__('company/companyLang.noJobListingListJob')}}
                </div>
            @endforelse
        </div>
    </div>
